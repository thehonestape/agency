...
const COMPONENTS_JSON_PATH = path.join(__dirname, '../src/data/components/tailwind-components.json');
...
const getImplementedComponentCount = () => {
  ...
  const categories = fs.readdirSync(COMPONENTS_DIR).filter(
    item => fs.statSync(path.join(COMPONENTS_DIR, item)).isDirectory()
  );
  
  for (const category of categories) {
    const categoryPath = path.join(COMPONENTS_DIR, category);
    
    const sections = fs.readdirSync(categoryPath).filter(
      item => fs.statSync(path.join(categoryPath, item)).isDirectory()
    );
...
3. **Ensure dark mode compatibility**:
   - Verify component looks correct in dark mode without explicit dark variants`;
    ...
    if (updatedContent.match(appUIRegex)) {
      ...
      updatedContent = updatedContent.replace(/(?<=This document tracks.*\n\n)/, `${appUISection}\n`);
    }
    ...
    if (updatedContent.match(ecommerceRegex)) {
      ...
      if (updatedContent.match(appUIRegex)) {
        updatedContent = updatedContent.replace(/(?<=## Application UI Components[\s\S]*?)(?=\n## |$)/, `\n\n${ecommerceSection}`);
      } else {
    ...
    if (updatedContent.match(marketingRegex)) {
      ...
      if (updatedContent.match(ecommerceRegex)) {
        updatedContent = updatedContent.replace(/(?<=## Ecommerce Components[\s\S]*?)(?=\n## |$)/, `\n\n${marketingSection}`);
