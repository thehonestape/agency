...
const createIndexFile = (dirPath, components) => {
  const imports = components.map(name => 
    `export * from './${toComponentFileName(name)}';`
  ).join('\n');
...
const generateComponentData = (category, sections) => {
  return {
    id: category.id,
    name: category.id.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
...
async function scrape() {
  ...
  try {
    ...
    for (const [section, components] of Object.entries(results)) {
      ...
      const indexContent = components.map((_, index) => {
...
async function scrapeCategory(page, category) {
  ...
  for (const section of sections) {
    ...
    const sectionDir = path.join(category.outputDir, 
    ...
    const componentsToProcess = COMPONENT_LIMIT === -1 
      ? section.components 
      : section.components.slice(0, COMPONENT_LIMIT);
    ...
    createIndexFile(sectionDir, section.components.map(c => c.name));
  }
...
function updateComponentRegistry(newComponentData) {
  const registryPath = path.join(__dirname, '../../src/data/components', 'tailwind-components.json');
...
async function scrapeAllSections(page) {
  ...
  for (const link of sectionLinks) {
    try {
      ...
      const sectionName = link.text.split(/\s+\d+/)[0].toLowerCase().replace(/\s+/g, '-');
        ...
        try {
          ...
          try {
            ...
            await page.evaluate((index) => {
              const elements = document.querySelectorAll('.component-preview, [role="group"], .preview, .example');
              if (elements[index]) {
                elements[index].dispatchEvent(new MouseEvent('click', {
                  bubbles: true,
                  cancelable: true,
                  view: window
                }));
          ...
          if (codeButton) {
            ...
            if (reactTab) {
              ...
              if (code) {
                ...
                await fs.writeFile(
                  `src/components/marketing/${fileName}`,
                  code,
                  'utf8'
                );
