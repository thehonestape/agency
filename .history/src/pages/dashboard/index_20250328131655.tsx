...
import { TremorColorPreview } from '@/components/ui/tremor/TremorColorPreview';
import { UIColorPreview } from '@/components/ui/tremor/UIColorPreview';
...
export default function Dashboard() {
  ...
  const [colorPreviewTab, setColorPreviewTab] = useState<'tremor' | 'ui'>('tremor');
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="mt-4 lg:mt-0 flex gap-2">
            ...
            >
              {showColorPreview ? 'Hide Colors' : 'Show Colors'}
            </button>
          </div>
        </div>
        ...
        {showColorPreview && (
          <div className="mt-6 space-y-4">
            <div className="flex justify-between items-center">
              <Title>Color System Preview</Title>
              <div className="flex gap-2">
                <button 
                  className={`px-4 py-2 rounded-md ${colorPreviewTab === 'tremor' ? 'bg-primary text-white' : 'bg-muted'}`}
                  onClick={() => setColorPreviewTab('tremor')}
                >
                  Tremor Colors
                </button>
                <button 
                  className={`px-4 py-2 rounded-md ${colorPreviewTab === 'ui' ? 'bg-primary text-white' : 'bg-muted'}`}
                  onClick={() => setColorPreviewTab('ui')}
                >
                  UI Theme Colors
                </button>
              </div>
