            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className="text-muted-foreground text-sm">Total AI costs</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className="text-muted-foreground text-sm">Total tokens used</div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className="text-muted-foreground text-sm">Total AI requests</div>
                </CardContent>
              </Card>
            </div>
                    ...
                    {dailyUsageData.length > 0 ? (
                      <LineChart
                        ...
                        customTooltip={({ payload }) => {
                          ...
                          const { date, cost } = payload[0].payload;
                          return (
                            <div className="rounded-lg border border-border bg-background p-2 shadow-sm">
                              <div className="text-base font-medium">{date}</div>
                              <div className="text-violet-500">${cost.toFixed(2)}</div>
                            </div>
                          );
                        }}
                      />
                  ...
                  <CardContent>
                    {dailyUsageData.length > 0 ? (
                      <BarChart
                        ...
                        customTooltip={({ payload }) => {
                          if (!payload?.[0]) return null;
                          const { date, tokens } = payload[0].payload;
                          return (
                            <div className="rounded-lg border border-border bg-background p-2 shadow-sm">
                              <div className="text-base font-medium">{date}</div>
                              <div className="text-cyan-500">{tokens.toLocaleString()} tokens</div>
                            </div>
                          );
                        }}
                      />
                    ...
                    {modelUsage.length > 0 ? (
                      <>
                        <DonutChart
                          ...
                          valueFormatter={(value) => `$${value.toFixed(2)}`}
                          className="h-72"
                          colors={["indigo", "blue", "cyan", "violet"]}
                          showAnimation={true}
                          showTooltip={true}
                          variant="pie"
                        />
