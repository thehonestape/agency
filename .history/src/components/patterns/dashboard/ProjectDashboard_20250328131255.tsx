...
export function ProjectDashboard({
  ...
  const artifactStatusData = [
    { name: 'Draft', value: artifactsByStatus.draft || 0, icon: FiEdit3, color: 'cyan' },
    { name: 'Review', value: artifactsByStatus.review || 0, icon: FiClock, color: 'yellow' },
    { name: 'Approved', value: artifactsByStatus.approved || 0, icon: FiCheck, color: 'lime' },
    { name: 'Archived', value: artifactsByStatus.archived || 0, icon: FiArchive, color: 'stone' },
  ];
  ...
  // Helper function to get CSS class for phase indicators
  const getPhaseIndicatorClass = (phase: ProjectPhase) => {
    return (
      (selectedProject?.currentPhase === 'discovery' && phase === 'discovery'
        ? 'bg-blue-500'
        : selectedProject?.currentPhase === 'definition' && phase === 'definition'
          ? 'bg-amber-500'
          : selectedProject?.currentPhase === 'design' && phase === 'design'
            ? 'bg-emerald-500'
            : 'bg-violet-500') + ' text-white h-2 w-2 rounded-full'
    );
  };
        ...
        <Grid numItemsMd={2} numItemsLg={2} className="gap-6">
          <Card>
            ...
            <DonutChart
              ...
              category="value"
              index="name"
              colors={['cyan', 'yellow', 'lime', 'violet']}
                ...
                <div className="flex h-full flex-col p-6">
                  ...
                  <div className="mt-6 flex flex-1 flex-col justify-between">
                    ...
                    <div className="mt-6 border-t border-border pt-4">
                      <Tracker
                        data={[
                          { color: 'lime', tooltip: 'Discovery' },
                          { color: 'lime', tooltip: 'Definition' },
                          { color: 'cyan', tooltip: 'Design - In Progress' },
