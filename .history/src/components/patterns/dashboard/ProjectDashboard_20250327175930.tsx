...
import { FiCheck, FiClock, FiEdit3, FiArchive, FiTrendingUp, FiAlertCircle, FiBriefcase, FiCalendar, FiChevronRight, FiInfo, FiLock } from 'react-icons/fi';
...
export function ProjectDashboard({
  ...
  const artifactsByStatus = artifacts.reduce((acc, artifact) => {
        ...
        insights = [
          {
            ...
            type: 'risk',
            title: 'Timeline Risk',
            description: 'The current approval rate of artifacts suggests a potential delay in the project timeline.',
            impact: 'medium',
            aiGenerated: true
          },
          {
            ...
            description: 'Increased client engagement in the review process could accelerate approvals.',
            impact: 'high',
            aiGenerated: true
          },
          {
            ...
            description: 'Consider allocating additional design resources to accelerate the current phase.',
            impact: 'medium',
            aiGenerated: true
          }
        ];
        ...
        <Card className="p-0 overflow-hidden">
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-800">
            {projects.slice(0, 5).map((project) => (
              <button
                key={project.id}
                className="w-full px-6 py-4 flex items-center justify-between hover:bg-gray-50 dark:hover:bg-gray-800/40 transition-colors text-left"
                onClick={() => onSelectProject(project)}
              >
                <div className="flex items-center">
                  <div className={`w-2 h-2 rounded-full mr-3 ${
                    project.currentPhase === 'discovery' ? 'bg-blue-500' :
                    project.currentPhase === 'definition' ? 'bg-amber-500' :
                    project.currentPhase === 'design' ? 'bg-emerald-500' :
                    'bg-purple-500'
                  }`} />
                  <div>
                    <Text className="font-medium">{project.name}</Text>
                    <Text className="text-xs text-gray-500">{project.currentPhase.charAt(0).toUpperCase() + project.currentPhase.slice(1)} Phase</Text>
                  </div>
                </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-3">
            ...
            <Badge color={
              selectedProject.currentPhase === 'discovery' ? 'blue' :
              selectedProject.currentPhase === 'definition' ? 'amber' :
              selectedProject.currentPhase === 'design' ? 'emerald' :
              'violet'
            }>
              {selectedProject.currentPhase.charAt(0).toUpperCase() + selectedProject.currentPhase.slice(1)} Phase
            </Badge>
          </div>
          <Text className="mt-1 text-gray-500">{selectedProject.description}</Text>
        </div>
                ...
                <div className="mt-4 space-y-4">
                  {isLoadingInsights ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center text-gray-500">
                      <FiInfo className="h-10 w-10 mb-3 text-gray-400" />
                  ...
                  {projectInsights.length > 0 && (
                    <div className="pt-3 mt-2 text-center border-t border-gray-200 dark:border-gray-800">
                      <Button
                        variant="light"
                        size="xs"
                        className="w-full"
                ...
                <div className="mt-6 grid gap-6 md:grid-cols-2">
                  {isLoadingInsights ? (
                    ...
                    Array.from({ length: 4 }).map((_, index) => (
                      <div key={index} className="animate-pulse p-4 border border-gray-200 dark:border-gray-800 rounded-lg">
                        <div className="flex items-start gap-3">
                    ...
                    <div className="flex flex-col items-center justify-center py-12 text-center text-gray-500 col-span-2">
                      <FiInfo className="h-12 w-12 mb-4 text-gray-400" />
                      <Text className="text-lg font-medium mb-2">No insights available</Text>
                      <Text className="max-w-md mb-4">
                        Generate AI insights based on your project's current status, timeline, and artifacts.
                      </Text>
                      <Button
                        variant="secondary"
                        onClick={generateProjectInsights}
...
const FiRefreshIcon = () => <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38"/></svg>; 