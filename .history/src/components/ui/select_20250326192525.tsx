...
>(({ className, children, size, width, state, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(selectTriggerVariants({ size, width, state, className }))}
    {...props}
  >
    {children}
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ref={ref}
        className={cn(selectContentVariants({ positionStyle, size: contentSize, className }))}
        position={position}
        {...props}
      >
  ...
  >
    <span className="absolute left-2 flex h-3.5 w-3.5 items-center justify-center">
      <SelectPrimitive.ItemIndicator>
        <Check className="h-4 w-4 text-accent-foreground animate-in zoom-in-50 duration-200" />
      </SelectPrimitive.ItemIndicator>
    </span>
