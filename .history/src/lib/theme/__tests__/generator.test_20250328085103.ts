...
describe('Theme Generator', () => {
  ...
  describe('Color Generation', () => {
    ...
    it('should generate color scales with proper lightness progression', () => {
      ...
      // Check brightness using RGB values
      const getBrightness = (color: string) => {
        const rgb = color.match(/\w\w/g)?.map(x => parseInt(x, 16)) || [];
        // Simple average of RGB values (0-255)
        return rgb.reduce((sum, val) => sum + val, 0) / 3;
      };

      const primary50Brightness = getBrightness(colors['primary-50']);
      const primary500Brightness = getBrightness(colors['primary-500']);
      const primary950Brightness = getBrightness(colors['primary-950']);

      // Higher numbers mean brighter colors in RGB
      expect(primary50Brightness).toBeGreaterThan(primary500Brightness);
      expect(primary500Brightness).toBeGreaterThan(primary950Brightness);
    });
