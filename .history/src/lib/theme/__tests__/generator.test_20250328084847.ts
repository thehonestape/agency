...
describe('Theme Generator', () => {
  ...
  describe('Color Generation', () => {
    ...
    it('should generate color scales with proper lightness progression', () => {
      ...
      // Check lightness progression using relative luminance
      const getLuminance = (color: string) => {
        const rgb = color.match(/\w\w/g)?.map(x => parseInt(x, 16) / 255) || [];
        return rgb.map(val => 
          val <= 0.03928 ? val / 12.92 : Math.pow((val + 0.055) / 1.055, 2.4)
        ).reduce((a, b) => a * 0.2126 + b * 0.7152 + b * 0.0722, 0);
      };

      const primary50Luminance = getLuminance(colors['primary-50']);
      const primary500Luminance = getLuminance(colors['primary-500']);
      const primary950Luminance = getLuminance(colors['primary-950']);

      expect(primary50Luminance).toBeGreaterThan(primary500Luminance);
      expect(primary500Luminance).toBeGreaterThan(primary950Luminance);
    });
