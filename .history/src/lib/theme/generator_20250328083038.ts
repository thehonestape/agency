...
export function generateTheme(config: ThemeConfig): Theme {
  ...
  const typography: ThemeTypography = {
    fontFamily: {
      ...
      sansDisplay: fontFamily.sansDisplay || DEFAULT_TYPOGRAPHY.fontFamily.sansDisplay,
      ...
      serifDisplay: fontFamily.serifDisplay || DEFAULT_TYPOGRAPHY.fontFamily.serifDisplay,
      mono: fontFamily.mono || DEFAULT_TYPOGRAPHY.fontFamily.mono,
      monoAlt: fontFamily.monoAlt || DEFAULT_TYPOGRAPHY.fontFamily.monoAlt,
      heading: fontFamily.heading || DEFAULT_TYPOGRAPHY.fontFamily.heading,
      body: fontFamily.body || DEFAULT_TYPOGRAPHY.fontFamily.body,
      code: fontFamily.code || DEFAULT_TYPOGRAPHY.fontFamily.code,
      ui: fontFamily.ui || DEFAULT_TYPOGRAPHY.fontFamily.ui,
    },
    fontSize: {
    ...
    paragraph: DEFAULT_TYPOGRAPHY.paragraph,
    heading: DEFAULT_TYPOGRAPHY.heading,
    list: DEFAULT_TYPOGRAPHY.list,
    link: DEFAULT_TYPOGRAPHY.link,
    code: DEFAULT_TYPOGRAPHY.code,
  };
...
export function generateThemeCSS(theme: Theme): string {
  ...
  Object.entries(theme.typography.fontSize).forEach(([key, [size, { lineHeight, letterSpacing, fontWeight }]]) => {
  ...
  // Paragraph settings
  Object.entries(theme.typography.paragraph).forEach(([key, value]) => {
    css += `  --paragraph-${key}: ${value};\n`;
  });

  // Heading settings
  Object.entries(theme.typography.heading).forEach(([key, value]) => {
    css += `  --heading-${key}: ${value};\n`;
  });

  // List settings
  Object.entries(theme.typography.list).forEach(([key, value]) => {
    css += `  --list-${key}: ${value};\n`;
  });

  // Link settings
  Object.entries(theme.typography.link).forEach(([key, value]) => {
    css += `  --link-${key}: ${value};\n`;
  });

  // Code settings
  Object.entries(theme.typography.code).forEach(([key, value]) => {
    css += `  --code-${key}: ${value};\n`;
  });
