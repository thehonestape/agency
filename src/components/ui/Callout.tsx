...
export function Callout({
  ...
  return (
    <div className={cn("my-6 flex rounded-lg p-4", styles[type].container, className)}>
      <IconComponent className="h-6 w-6 flex-none text-accent-foreground" />
      <div className="ml-4 flex-auto">
        <p className={cn("m-0 font-medium text-lg", styles[type].title)}>
          {title}
        </p>
        <div className={cn("prose mt-2", styles[type].body)}>
          {children}
        </div>
      </div>
    </div>
